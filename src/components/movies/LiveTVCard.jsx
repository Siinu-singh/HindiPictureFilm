...
export default function LiveTVCard({ channel }) {
  return (
    ...
    >
      <motion.div
        className="relative group rounded-lg sm:rounded-xl overflow-hidden shadow-lg hover:shadow-primary/40 transition-all duration-300 bg-card border border-transparent group-focus-within:ring-2 group-focus-within:ring-primary group-focus-within:border-primary/70 h-full flex flex-col"
        variants={cardVariants}
        whileHover={{
          scale: 1.03, // Reduced scale
          y: -3, // Reduced y offset
          boxShadow: "0px 8px 15px hsla(var(--primary-rgb), 0.25), 0px 0px 10px hsla(var(--primary-rgb), 0.15)", // Adjusted shadow
      ...
      >
        <div className="relative w-full h-32 sm:h-40 md:h-48 bg-muted/50 flex items-center justify-center overflow-hidden">
          {channel.logoUrl ? (
            <Image
              src={channel.logoUrl}
              alt={`${channel.channelName} logo`}
              width={100} // Adjusted for smaller cards
              height={60}  // Adjusted for smaller cards
              className="object-contain transition-transform duration-300 group-hover:scale-110"
              data-ai-hint={channel.dataAiHintLogo || "channel logo"}
            />
          ) : (
            <div className="w-full h-full flex items-center justify-center bg-secondary">
              <Tv className="w-12 h-12 sm:w-16 sm:h-16 text-muted-foreground/50" />
            </div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            <PlayCircle className="w-10 h-10 sm:w-12 sm:h-12 text-white/80 group-hover:text-white" />
          </div>
        </div>
        <div className="p-3 sm:p-4 flex flex-col flex-grow">
          ...
          <p className="text-xs text-muted-foreground mt-1 truncate h-8" title={channel.description}>
            {channel.description}
          </p>
